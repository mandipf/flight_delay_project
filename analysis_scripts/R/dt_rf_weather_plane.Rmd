---
title: "Decision Trees - Weather and plane variables"
...
```{r}
rf_tune_save <- "saved_models/weather_plane_rf_tune.RDS"
```
...
delay_trim_df <- ewr_delays_df %>%  
  select(delay_flag, wind_dir, wind_speed, pres, tavg, prcp,  
         visib, snow, snwd,
         distance, nose_dir, carrier, type, seats) %>% 
  drop_na(seats)
```
...
```{r}
delay_test_pred <- delay_test %>%
  mutate(pred = predict(rf_classifier,
                        data = delay_test)$predictions)
...
```{r}
delay_tuned_test_pred <- delay_test %>%
