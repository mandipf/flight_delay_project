---
title: "Decision Trees - Weather related variables only (balanced data)"
...
```{r}
rf_tune_save <- "saved_models/weather_only_bal_rf_tune.RDS"
```
...
```{r}
# calculate weight for rf_classifier
...
```{r}
# untuned model for comparison using latest rose_test 
delay_test_pred <- rose_test %>%
  mutate(pred = predict(rf_classifier,
                        data = rose_test)$predictions)
